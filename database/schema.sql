CREATE DATABASE IF NOT EXISTS basaveshwar_hotel;
...
-- Note: daily_sales can often be calculated dynamically using aggregate queries on `payments` and `orders`. 
-- We'll rely on dynamic calculation for real-time reporting, but if a materialized view is needed later, we can add a table.
...
-- Insert Default Users (Password is 'password123' hashed using bcrypt)
-- Waiters
